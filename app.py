...
with right_col:
    ...
    with age_col:
        ...
        if age_counts:
            ...
            fig_age = go.Figure(go.Pie(
                ...
                textinfo="value", textfont=dict(size=10, color="#ffffff"),
                hovertemplate="%{label}: %{value}<extra></extra>",
            ))
            fig_age.update_layout(
                height=AGE_H, autosize=True,
                paper_bgcolor=C["bg"], margin=dict(l=4, r=4, t=4, b=30),
                showlegend=True,
                legend=dict(orientation="h", yanchor="bottom", y=-0.15,
                           xanchor="center", x=0.5, font=dict(size=9, color=C["text"])),
            )
            st.plotly_chart(fig_age, use_container_width=True,
                            config={"displayModeBar": False}, key="age_chart")

    act_col, pie_col = st.columns([1.4, 1])
    ...
    with pie_col:
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            hole=0.55,
            marker=dict(colors=[C["fire_yellow"], C["fire_orange"]], line=dict(color=C["bg"], width=2)),
